...
public class NetworkHandler implements INetty, PostConfigHandler, PreSaveHandler {
    ...
    public void handleCrossClientData(UUID uuid, JsonHolder jsonHolder) {
        ...
        if (type.equalsIgnoreCase("fortnite_default_dance")) {
        ...
        }else if (type.equalsIgnoreCase("fortnite_hype_dance")) {
            Hyperium.INSTANCE.getHandlers().getFortniteHypeDance().startAnimation(uuid);
            Hyperium.INSTANCE.getHandlers().getFortniteHypeDance().getStates().put(uuid, System.currentTimeMillis());
            ...
            } else if (Minecraft.getMinecraft().currentScreen instanceof BibsGui) {
                ((BibsGui) Minecraft.getMinecraft().currentScreen).updatePurchases();
            } else {
